//ejercicio F
public class DoubleHashF {
    private Libro[] hashArray;
    private int arraySize;
    private Libro nonItem;

    public DoubleHashF(int size) {
        arraySize = size;
...
//insert a Libro, si se realiza un sondeo se le suma +1,de ahí +2 y asdf
    public void insert(long key, Libro item) {
        ...
        while (hashArray[(int) hashVal] != null && hashArray[(int) hashVal].getPleg() != -1) {
            hashVal += stepSize;
            hashVal %= arraySize;
        }
...
//-------------------------------------------------------------
    ...
    public Libro delete(int val) {
        ...
        while (hashArray[(int) hashVal] != null) {
            if (hashArray[(int) hashVal].getPleg() == key) {
